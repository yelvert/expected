...
module Expected
  RSpec.describe Matchers do
    context 'subject is a module' do
      subject do
        Module.new do
          self::FOO = 'bar'
          self::BAZ = {}.freeze
      ...
      it { is_expected.to have_constant(:BAZ).of_type(Hash) }
      it { is_expected.to have_constant(:BAZ).of_type(Hash).with_value({}) }
      it { is_expected.not_to have_constant(:BAR) }
      it { is_expected.not_to have_constant(:FOO).with_value('bad value') }
      it { is_expected.not_to have_constant(:FOO).of_type(Array) }
    end

    context 'subject is a class' do
      subject do
        Class.new do
          self::FOO = 'bar'
          self::BAZ = {}.freeze
  ...
  module Matchers
    RSpec.describe HaveConstantMatcher do
      ...
      describe '#matches?(subject)' do
        ...
        let(:matches_subject_class_name) { random_const_name }
        let(:matches_subject) { Object.const_set(matches_subject_class_name, Class.new).new }

        after(:example) { Object.send(:remove_const, matches_subject_class_name) }
      ...
      describe '#description' do
        ...
        describe '#constant_exists?' do
          it 'should return true if the subject has the constant defined' do
            expect(subject.subject).to receive(:const_defined?).with(subject.name).and_return(true)
          ...
          it 'should return false and set the failure message if the subject does not have the constant defined' do
            expect(subject.subject).to receive(:const_defined?).with(subject.name).and_return(false)
            expect(subject.instance_variable_defined?(:@failure)).to be_falsy
