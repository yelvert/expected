...
module Expected
  module Matchers
    RSpec.describe ExtendModuleMatcher do
      shared_examples :extend_module do |expected_subject, good, bad|
        good.each do |klass|
          it { is_expected.to extend_module(klass) }
          it {
            expect { is_expected.not_to extend_module(klass) }.to(
              fail_with_message("Did not expect <#{expected_subject.inspect}> to extend <#{klass.inspect}>")
            )
          }
        end
        bad.each do |klass|
          it { is_expected.not_to extend_module(klass) }
          it {
            expect { is_expected.to extend_module(klass) }.to(
              fail_with_message("Expected <#{expected_subject.inspect}> to extend <#{klass.inspect}>")
            )
          }
        end
      end
      ...
      describe '#matches?(subject)' do
        ...
        it 'should return true if the subject extends the expected_module' do
          included_modules = double
          singleton_class = double
          expect(matches_subject).to receive(:singleton_class).and_return(singleton_class)
          expect(singleton_class).to receive(:included_modules).and_return(included_modules)
        ...
        it 'should return false if the subject does not extends the expected_module' do
          included_modules = double
          singleton_class = double
          expect(matches_subject).to receive(:singleton_class).and_return(singleton_class)
          expect(singleton_class).to receive(:included_modules).and_return(included_modules)
      ...
      describe '#description' do
        it 'should have the expected_module' do
          expect(subject.description).to eql("extend_module: <#{subject.expected_module.inspect}>")
        end
      end
      ...
      describe '#expectation' do
        it 'should have the expected ancestor' do
          ...
          expect(subject.send(:expectation)).to(
            eql("<#{subject.subject.inspect}> to extend <#{expected_module.inspect}>")
          )
        end
      end
    end
